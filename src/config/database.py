...
class PostgresDatabaseManager:
    ...
    def add_position(
        ...
        with self.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO positions (symbol, quantity, entry_price, current_price, unrealized_pnl, entry_time)
                    VALUES (%s, %s, %s, %s, %s, %s)
                    RETURNING id
                    """,
    ...
    def add_order(
        ...
        with self.get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO orders (order_id, symbol, side, quantity, price, order_type, status, submitted_time)
    ...
    def get_daily_trade_stats(self, date_str: Optional[str] = None) -> Dict[str, Any]:
        if date_str is None:
    ...
    if _db_manager is None:
        try:
            if backend:
                ...
                backend_name = settings.database.backend.lower()
                if dsn is None:
            ...
            if backend_name == "postgres":
                ...
                _db_manager = PostgresDatabaseManager(dsn)
            else:
